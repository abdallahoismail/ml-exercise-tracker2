...
def find_data_file():
    """Find the data file regardless of current working directory"""
    script_dir = os.path.dirname(os.path.abspath(__file__))

    possible_paths = [
        # From project root
        os.path.join(script_dir, "data", "interim", "01_preprocessed_data.pkl"),
    ...
    raise FileNotFoundError(
        f"Could not find 01_preprocessed_data.pkl in any of these locations: {possible_paths}"
    )
...
def test_production_ready_pipeline():
    ...
    # Load test data with proper path handling
    try:
    ...
    for test_case in test_sets[:5]:  # Test first 5 for brevity
...
def quick_test():
    """
    Quick test function for Jupyter/interactive use
    """
    ...
    try:
        ...
        print(f"Testing Set 1 - Actual: {actual_label}")
        ...
        return results["exercise"] == actual_label
